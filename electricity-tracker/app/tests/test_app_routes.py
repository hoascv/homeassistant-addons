...
def test_stats_empty_db(client):
    ...
    data = res.get_json()
    assert data["counts"] == {"prices": 0, "consumption": 0, "saveeye_samples": 0, "easee_samples": 0}
    assert data["total"] == 0
...
def test_export_shape(client):
    ...
    assert set(data["tables"].keys()) == {"prices", "consumption", "saveeye_samples", "easee_samples"}
