...
def app_server(_data_dir):
    ...
    env.update({
        "JOURNAL_DB_PATH": str(_data_dir / "journal.db"),
        "JOURNAL_OPTIONS_PATH": str(_data_dir / "options.json"),
        "JOURNAL_PORT": str(port),
    })
    ...
    request = urllib.request.Request(f"{base_url}/", headers=INGRESS_HEADERS)
